*****************************************************************;
* For HF 30-day readmission measure using Medicare claim data.  *;
...
%LET EST=R.&CONDITION._READM_EST;
	...
	/* HF READMISSION MODEL VARIABLES */
...
data INDEX;
	...
	IF index_condition="CHF" THEN HF_CASE=1;
	ELSE HF_CASE=0;
...
/* IDENTIFY AND COMBINE TWO ADJACENT HF ADMISSIONS (disch1=admit2), USE DISCHARGE DATE
	...
	THIS WORKS WHEN THERE ARE MORE THAN TWO ADJACENT HF ADMISSIONS. */
...
if (admit <= lag(disch) <= disch) and lag(provid)=provid
	and lag(hicno)=hicno
	and lag(HF_case)=HF_case=1 then combine0=1;
else combine0=0;
RUN;
...
if (admit <= lag(admit) <= disch) and 
	lag(provid)=provid
	and lag(hicno)=hicno
	and lag(HF_case)=HF_case=1 then combine=1;
else combine=0;
...
/* APPLY THE FOLLOWING INCLUSION AND EXCLUSION CRITERIA:
	HF_CASE=1, AGE >=65, DEAD=0, PREMO=12, POSTMOD=1, 2, 3,
...
IF (HF_CASE=1) AND (TRANS_COMBINE=0) AND (AGE65=1) AND (DEAD=0)
...
proc freq data=ALL;
	table HF_CASE TRANS_COMBINE AGE65 DEAD PRIOR12 POST1 AMA;
run;
...
DATA _ALL;
MERGE ALL (IN=A WHERE=(HF_CASE=1)) INDEXADMISSION (IN=B KEEP=HICNO CASE);
BY HICNO CASE;
...
IF _index_condition ="CHF" THEN RADM_HF=1;
ELSE RADM_HF=0;
...
IF RADM30=1 AND RADM_hf=1 AND INTERVAL=0 AND SAME=1 THEN TRANS_BUNDLE=1;
ELSE TRANS_BUNDLE=0; 
...
proc freq data=R.collapsed_&CONDITION.;
	tables radm30 radm45 dd30 radm30*_factype hf_:;
run;
