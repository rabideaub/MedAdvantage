...
%LET CDIAG=(index_condition="Stroke");
...
data INDEX;
	...
	IF index_condition="Stroke" THEN SK_CASE=1;
	ELSE SK_CASE=0;
...
/* IDENTIFY AND COMBINE TWO ADJACENT SK ADMISSIONS (disch1=admit2), USE DISCHARGE DATE
...
/* INCLUSIOIN CRITERIA: FFS, SK CASE, AGE GE 65, WITH 12-MONTH HISTORY,
   EXCLUSION  CRITERIA: TRANSFER OUT, IN HOSPITAL DEATH, WITHOUT >= 1 MOTNH POST,
						AMA */

IF (SK_CASE=1) AND (TRANS_COMBINE=0) AND (AGE65=1) AND (DEAD=0)
...
proc freq data=ALL;
	table SK_CASE TRANS_COMBINE AGE65 DEAD PRIOR12 POST1 AMA;
run;
...
IF _index_condition ="Stroke" THEN RADM_SK=1;
ELSE RADM_SK=0;
...
IF RADM30=1 AND RADM_SK=1 AND INTERVAL=0 AND SAME=1 THEN TRANS_BUNDLE=1;
ELSE TRANS_BUNDLE=0; 
...
proc freq data=R.collapsed_&CONDITION.;
	tables radm30 radm45 dd30 radm30*_factype morbid_obesity stroke_: ;
run;
