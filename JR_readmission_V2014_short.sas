*****************************************************************;
* For JR 30-day readmission measure using Medicare claim data.  *;
...
%LET EST=R.&CONDITION._READM_EST;
	...
	/* JR READMISSION MODEL VARIABLES */
...
data INDEX;
	...
	IF &&DIAGSELECT_&DX. THEN JR_CASE=1;
	ELSE JR_CASE=0;
...
/* IDENTIFY AND COMBINE TWO ADJACENT JR ADMISSIONS (disch1=admit2), USE DISCHARGE DATE
	...
	THIS WORKS WHEN THERE ARE MORE THAN TWO ADJACENT JR ADMISSIONS. */
...
if (admit <= lag(disch) <= disch) and lag(provid)=provid
	and lag(hicno)=hicno
	and lag(JR_case)=JR_case=1 then combine0=1;
else combine0=0;
RUN;
...
if (admit <= lag(admit) <= disch) and 
	lag(provid)=provid
	and lag(hicno)=hicno
	and lag(JR_case)=JR_case=1 then combine=1;
else combine=0;
...
/* APPLY THE FOLLOWING INCLUSION AND EXCLUSION CRITERIA:
	JR_CASE=1, AGE >=65, DEAD=0, PREMO=12, POSTMOD=1, 2, 3,
...
/* INCLUSIOIN CRITERIA: FFS, JR CASE, AGE GE 65, WITH 12-MONTH HISTORY,
   EXCLUSION  CRITERIA: TRANSFER OUT, IN HOSPITAL DEATH, WITHOUT >= 1 MOTNH POST,
						AMA */

IF (JR_CASE=1) AND (TRANS_COMBINE=0) AND (AGE65=1) AND (DEAD=0)
...
proc freq data=ALL;
	table JR_CASE TRANS_COMBINE AGE65 DEAD PRIOR12 POST1 AMA;
run;
...
/* KEEP ADMS THAT ARE AT LEAST 180 DAYS APART */
DATA ONEPLUS180DAY;
	SET ONEPLUS;
	BY HICNO;
RETAIN BASEDATE 0;
IF FIRST.HICNO THEN DO;
	...
	DIFF=0;
	END;
ELSE IF (ADMIT - BASEDATE) <=180 THEN DO;
	KEEP=0;
	DIFF=ADMIT - BASEDATE;
	END;
ELSE IF (ADMIT - BASEDATE) > 180 THEN DO;
...
DATA _ALL;
MERGE ALL (IN=A WHERE=(JR_CASE=1)) INDEXADMISSION (IN=B KEEP=HICNO CASE);
BY HICNO CASE;

IF A;

ATTRIB _180DAYS LABEL='180 DAYS WINDOW EXCLUSION' LENGTH=8.;
ATTRIB SAMPLE LABEL='ADMISSION IN FINAL SAMPLE' LENGTH=8.;

if criteria=1 and not b then _180Days=1;
else _180Days=0;
...
MERGE _ALL (IN=A)
	 post (IN=B /*where=(va_obs_stay=0)*/ KEEP=HICNO ADMIT DISCH PROVID /*va_obs_stay*/ DIAG1 CASE planned 
			factype sslssnf pmt_amt tot_chg DRGCD DDEST index_condition _39: _20a _20b _21a _21d _22
...
IF _index_condition ="LEJR" THEN RADM_JR=1;
ELSE RADM_JR=0;
...
IF RADM30=1 AND RADM_JR=1 AND INTERVAL=0 AND SAME=1 THEN TRANS_BUNDLE=1;
ELSE TRANS_BUNDLE=0; 
...
DATA sample;
	...
	bilateral_hip=0;
	if proc1 in('8151','8152') then do;
		%macro loop_bilateral_hip(n);
			%do j=2 %to &n.;
				if proc&n. in('8151','8152') then bilateral_hip=1;
			%end;
		%mend;
		%loop_bilateral_hip(25);
	end;
	if proc1 in('8154') then do;
		%macro loop_bilateral_knee(n);
			%do j=2 %to &n.;
				if proc&n. in('8154') then bilateral_hip=1;
			%end;
		%mend;
		%loop_bilateral_knee(25);
	end;
...
proc freq data=R.sample_&CONDITION.;
	tables radm30 year _factype _factype*year bilateral_hip;
run;
...
%macro factype(var);
	...
	data sample_&condition. (keep=hicno case hicno_case case_count _factype radm30 radm45 dd30 cstshr:
				      ADMIT DISCH PROVID PROC1 pmt_amt tot_chg DRGCD DDEST sslssnf premo
				      _ADMIT _DISCH _PROVID _pmt_amt _tot_chg _DRGCD _DDEST  cstshr:
				      _sslssnf DEATH year DIAG1-DIAG25 ma_claim entry_dt disch_dt drop_obs _39: _20a _20b _21a _21d _22
				      RACE MALE ZIP RTI_RACE_CD AGE BUYIN_MO first_rug disch_cd mds_los snf_los irf_los RUG4 spclunit
				      prior_month_cov: dis_month_cov1 dis_month_cov2 dis_month_cov3 _21a _21d _22 ghopdcd qtr_assess
	...
	data sample_&condition._&var. (drop=radm30 radm45 dd30 
		...
		by hicno_case;
			...
			array &var.z22_{&N.}  $ &var._22_1-&var._22_&N.;
...
proc freq data=R.collapsed_&CONDITION.;
	tables radm30 radm45 dd30 radm30*_factype bilateral_hip: morbid_obesity ;
run;
